...
export default function ArventoRaporPage() {
  ...
  // İlk yükleme yapıldı mı — sayfa-seviyesi "Yükleniyor" ekranı YALNIZ ilk açılışta gösterilsin. Tarih değişince
  // loadKayitlar loading'i tekrar true yapıyordu → tüm sayfa (tam ekran harita dahil) unmount olup TAM EKRAN
  // KAPANIYORDU. İlk yüklemeden sonra sayfa karartılmaz; her sekme kendi yüklemesini gösterir.
  const ilkYuklendiRef = useRef(false);
  ...
  const loadKayitlar = useCallback(async () => {
    ...
    } finally { if (benimNo === kayitYukRef.current.no) { setLoading(false); ilkYuklendiRef.current = true; } }
  }, [baslangic, bitis]);
  ...
  // ŞOFÖR OVERRIDE'lı cihaz eşlemesi — canlı harita popup'larında, Tanımlamalar'daki "Şoför" kolonuna
  // yazılan ad cihazdan (Excel) gelen adın YERİNE gösterilir ("-" = isim gizlenir).
  const canliCihazMapEfektif = useMemo<CihazMap>(() => {
    if (surucuOverride.size === 0) return canliCihazMap;
    const m: CihazMap = new Map(canliCihazMap);
    for (const [node, c] of m) {
      const o = c.plaka ? surucuOverride.get(plakaNorm(c.plaka)) : undefined;
      if (o !== undefined) m.set(node, { ...c, surucu: o === "-" ? null : o });
    }
    return m;
  }, [canliCihazMap, surucuOverride]);
  // Plaka(norm) → ARAÇ ETİKETİ (chip'lerde "İş Makinesi/cins" yerine gösterilir). ÖNCELİK: Tanımlamalar'daki
  // araç etiketi (arvento_cihaz.model); yoksa araclar.model'e düşer.
  const modelMap = useMemo(() => {
    const m = new Map<string, string | null>();
    for (const [p, ps] of plakaSantiye.entries()) m.set(p, ps.model ?? null); // fallback: araclar.model
    for (const c of canliCihazMap.values()) { // öncelik: araç etiketi (arvento_cihaz.model)
      if (c.plaka && c.model && c.model.trim()) m.set(plakaNorm(c.plaka), c.model);
    }
    return m;
  }, [plakaSantiye, canliCihazMap]);
  ...
  // YALNIZ ilk yüklemede tam-sayfa "Yükleniyor" göster. Sonraki tarih değişimlerinde sayfa karartılmaz →
  ...
  // TAM EKRANDA SEKME PANELİ — sayfanın sekme çubuğu tam ekranda görünmediği için, sekme değiştirmek
  // isteyen kullanıcı tam ekrandan çıkmak zorunda kalıyordu. Bu panel harita kapsayıcısının İÇİNE
  // render edilir (sekmePanel prop'u); normal modda gizlidir, yalnız tam ekranda solda görünür.
  // Tam ekran <body> sınıfıyla tutulduğu için sekme değişince mod BOZULMAZ — harita küçülmez.
  const sekmePanel = (
    <div className="harita-sekme-panel">
      {([["guzergah", "Reglaj"], ["genel", "Stabilize"], ["serme", "Serme"], ["sikistirma", "Sıkıştırma"], ["tumu", "Tümü"]] as const).map(([key, label]) => (
        <button key={key} type="button"
          onClick={() => { if (aktifSekme !== key) setAktifSekme(key); }}
          className={`px-2 py-1.5 text-[11px] font-semibold rounded-md whitespace-nowrap transition-colors select-none ${
            aktifSekme === key ? "bg-[#F97316] text-white" : "bg-white/90 text-gray-700 hover:bg-white"
          }`}>
          {label}
        </button>
      ))}
  ...
  return (
    <div>
      ...
      {/* Filtreler + özet — YALNIZ "Araç Çalışma Raporu"nda üstte. Harita sekmelerinde (İş Makineleri/Reglaj/
          Stabilize/Serme/Sıkıştırma/Tümü) tarih navigatörü sağdaki buton alanına taşındı. */}
      {aktifSekme === "calisma" && (
          ...
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div>
              ...
              <p className="text-xs text-gray-400 max-w-2xl">
                ...
                <br /><strong>Şoför</strong> kolonu: dolu ise sitede Arvento&apos;dan gelen sürücü adının yerine bu gösterilir;
                boş = Arvento adı; <strong>tire (-)</strong> yazarsanız isim hiç gösterilmez (şoförsüz araç).
                Değişiklikler &quot;Atamaları Kaydet&quot; ile kaydedilir.
              </p>
            </div>
            {yDuzenle && (
