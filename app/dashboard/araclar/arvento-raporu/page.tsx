...
import { getArventoTarihler, getArventoRaporByTarih } from "@/lib/supabase/queries/arvento";
...
import { Satellite, Search, Upload, FileSpreadsheet, RefreshCw, Gauge, Route, Clock } from "lucide-react";
...
export default function ArventoRaporPage() {
  ...
  const loadKayitlar = useCallback(async () => {
    ...
    try {
      const k = await getArventoRaporByTarih(seciliTarih);
      setKayitlar(k);
    } catch (err) {
  ...
  // Gece cron'unu elle tetikle (link yoksa manuel yükleme önerilir)
  ...
  const ozet = useMemo(() => {
    ...
    const calisan = filtrelenmis.filter((k) => (k.hareket_sn ?? 0) > 0 || (k.mesafe_km ?? 0) > 0).length;
    const toplamHareket = filtrelenmis.reduce((s, k) => s + (k.hareket_sn ?? 0), 0);
    return { sayi: filtrelenmis.length, calisan, toplamKm, toplamHareket };
  }, [filtrelenmis]);

  function exportExcel() {
    const headers = ["Plaka", "Sürücü", "Marka", "Model", "Mesafe (km)", "Hareket Süresi", "Kontak Açık", "Rölanti", "Maks Hız (km/s)"];
    const data = filtrelenmis.map((k) => [
      k.plaka, k.surucu ?? "", k.marka ?? "", k.model ?? "",
      k.mesafe_km ?? "", formatSure(k.hareket_sn), formatSure(k.kontak_sn), formatSure(k.rolanti_sn),
      k.maks_hiz ?? "",
    ]);
    const ws = XLSX.utils.aoa_to_sheet([headers, ...data]);
    ws["!cols"] = [{ wch: 14 }, { wch: 18 }, { wch: 14 }, { wch: 14 }, { wch: 12 }, { wch: 14 }, { wch: 14 }, { wch: 12 }, { wch: 14 }];
      ...
      <div className="bg-white rounded-lg border p-3 mb-4 flex flex-wrap items-end gap-3">
        ...
        <div className="ml-auto flex items-end gap-4">
          <div className="text-xs text-gray-600 text-right leading-relaxed">
            ...
            <div>Toplam: <strong className="text-[#1E3A5F]">{formatKm(ozet.toplamKm)} km</strong> · Hareket: <strong>{formatSure(ozet.toplamHareket)}</strong></div>
          </div>
