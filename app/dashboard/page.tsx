...
export default function DashboardPage() {
  ...
  //   • SÜRESİ GEÇMİŞ: kalanGun < 0 — araç trafiğe çıkamaz.
  // Eskiden yalnız süresi geçmişler vardı; uyarı ancak iş işten geçtikten sonra çıkıyordu.
  // yaklasanlar 15 kayıtla sınırlı ama kalanGun'a göre sıralı olduğundan bu iki grup her
  // zaman listenin başında yer alır.
  const suresiGecmisler = useMemo(
    () => yaklasanlar.filter((y) => y.kalanGun < 0),
    [yaklasanlar],
  );
  const sonGunKalanlar = useMemo(
    () => yaklasanlar.filter((y) => y.kalanGun >= 0 && y.kalanGun <= 1),
    [yaklasanlar],
  );
  const seritKayitlari = useMemo(
  ...
  return (
    <div>
      ...
      {wg("sigorta_muayene") && seritKayitlari.length > 0 && (
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-red-700">
                {suresiGecmisler.length > 0
                  ? "Süresi geçmiş araçlar trafiğe çıkamaz. "
                  : "Son gün — bugün işlem yapılmazsa araç trafiğe çıkamaz. "}
                Aşağıdaki &quot;Yaklaşan Sigorta &amp; Muayene&quot; bölümünden poliçe girebilir veya teklif isteyebilirsiniz.
              </div>
            </div>
          </div>
          <ul className="mt-2 space-y-1 pl-9 text-sm">
            {seritKayitlari.map((y) => (
              <li key={`${y.aracId}-${y.field}`} className="flex flex-wrap items-center gap-x-2 gap-y-1">
                ...
                {y.kalanGun < 0 ? (
                  ...
                  <span className="rounded bg-amber-200 px-1.5 py-0.5 text-[11px] font-semibold text-amber-900">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {wg("bordro_ozet") ? <div className="bg-white rounded-lg border p-4 md:col-span-2 lg:col-span-4 lg:order-9">
          ...
          {bordroLoading ? (
            ...
            <div>
              {/* containerClassName: shadcn Table varsayılanı "overflow-auto max-h-[75vh]" —
                  bu iç dikey scrollbar üretiyordu. max-h-none ile yükseklik sınırı kalkar
                  (tablo kaç iş varsa o kadar uzar, iç dikey scrollbar olmaz). Masaüstünde
                    ...
                    {cevapsizlar.length > 0 && (
                      <details className="rounded-lg border border-dashed border-gray-300 px-2.5 py-1.5">
                        ...
                        {cevapsizlar.map((acente) => {
                          ...
                          return (
                            <div key={acente} className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_130px] gap-2 items-end">
                              <span className="text-xs font-medium text-gray-700 sm:pb-2">{acente}</span>
                              <select value={teklifFirma[acente] ?? ""} style={{ fontSize: "16px" }}
                                className="h-10 sm:h-9 w-full rounded-md border border-input bg-white px-2 text-sm outline-none focus:border-ring"
                              ...
                              <input inputMode="decimal" value={teklifTutar[acente] ?? ""} disabled={!firmaSecili}
                                ...
                                placeholder={firmaSecili ? "tutar" : "önce firma"} style={{ fontSize: "16px" }}
                                className="h-10 sm:h-9 w-full text-right rounded-md border border-input bg-transparent px-2 text-sm tabular-nums outline-none focus:border-ring disabled:bg-gray-50 disabled:text-gray-300" />
                            </div>
                          );
                        })}
                        </div>
