...
type Hareket =
  | {
      ...
      duzeltme: boolean; // düzeltme kaydı mı (km_saat yok, ± litre hisse dağıtımı)
      notu: string | null;
...
type TabloSatir = {
  ...
  disYakit?: boolean;    // efektif dış-yakıt (otomatik fark>menzil veya manuel) → D rozeti + ortalamadan dışlanır
  disSupheli?: boolean;  // şantiye görünümünde D şüpheli (fark başka şantiyelerden) → "Ş" rozeti
...
// Düzeltme miktarı parse — ± (negatif) olabilir. Boş/geçersiz ise NaN döner
// (parseParaInput 0 döndürdüğü için "boş" ile "0" ayrılamaz; düzeltmede ayırmak gerekir).
function parseDuzMiktar(formatted: string): number {
  const t = (formatted ?? "").trim();
  if (!t || t === "-") return NaN;
  const temiz = t.replace(/\./g, "").replace(",", ".");
...
function YakitPageContent() {
  ...
  const aracGenelOrt = useMemo(() => {
    ...
    for (const [aracId, kayitlar] of byArac.entries()) {
      // km/saat GİRİLMEMİŞ (0 veya negatif) kayıtlar dışlanır — yoksa bir sonraki
      // aralığın farkı (sonraki.km − 0 = kocaman) ortalamayı bozar.
      ...
      // Efektif dış-yakıt aralığı dışlanır: manuel (true/false) öncelikli; null ise
      // OTOMATİK (mesafe menzili aşıyorsa). Fark görünüme bağlı (şantiye-farkı/araç-farkı).
      let toplamLt = 0;
      let toplamMesafe = 0;
      for (let i = 1; i < sirali.length; i++) {
        ...
        const disYakit = buK.dis_yakit_oncesi === true ? true
          : buK.dis_yakit_oncesi === false ? false
          : (menzil > 0 && mesafe > menzil);
        if (disYakit) continue; // dış-yakıt aralığı ortalamaya katılmaz
  ...
  // (şantiye filtresi aktifken fark/anlık ortalama şantiye bazlı hesaplansın).
  function oncekiAracKayit(aracId: string, tarih: string, saat: string, mevcutId?: string, santiyeId?: string): AracYakit | null {
    ...
    for (const y of yakitKayitlari) {
      ...
      if ((y.km_saat ?? 0) <= 0) continue; // km/saat girilmemiş kayıt önceki sayılmaz
  ...
  const tumHareketler = useMemo<Hareket[]>(() => {
    ...
    for (const y of yakitKayitlari) {
      hs.push({
        ...
        dis_yakit_oncesi: y.dis_yakit_oncesi ?? null, // null = otomatik (menzile göre)
        duzeltme: y.duzeltme === true,
        notu: y.notu,
  ...
  const tabloSatirlari = useMemo<TabloSatir[]>(() => {
    ...
    const filtrelenmis = tumHareketler.filter((h) => {
      // Düzeltme kayıtları SADECE yöneticiye gösterilir (yönetici dışı tüm kullanıcılar gizler).
      if (h.tip === "arac_yakit" && h.duzeltme && !isYonetici) return false;
      if (izinliSantiyeler) {
    ...
    for (const h of filtrelenmis) {
      ...
      if (h.tip === "arac_yakit") {
        ...
        // Şantiye filtresi aktifse fark/anlık ortalama o şantiyedeki önceki doluma göre.
        const onceki = oncekiAracKayit(h.arac_id, h.tarih, h.saat, h.id, filtreSantiyeId || undefined);
        if (onceki) {
          const fark = h.km_saat - onceki.km_saat; // görünüm farkı (şantiye seçiliyse şantiye-farkı)
          satir.fark = fark;
          const menzil = arac?.depo_menzil ?? 0;
          // Efektif dış-yakıt: manuel (true/false) öncelikli; null ise OTOMATİK
          // (fark, 1 depo menzilini aşıyorsa dış yakıt). Ortalamadan dışlanır.
          const disYakit = h.dis_yakit_oncesi === true ? true
            : h.dis_yakit_oncesi === false ? false
            : (menzil > 0 && fark > 0 && fark > menzil);
          satir.disYakit = disYakit;
          // Ş (şüphe): şantiye filtreliyken D var AMA aracın GLOBAL farkı menzili AŞMIYOR
          // → bu şantiye-farkı başka şantiyelerdeki dolumlardan kaynaklı, D yanıltıcı olabilir.
          if (filtreSantiyeId && disYakit && menzil > 0) {
            const oncekiArac = oncekiAracKayit(h.arac_id, h.tarih, h.saat, h.id); // global (şantiyesiz)
            if (oncekiArac) {
              const aracFark = h.km_saat - oncekiArac.km_saat;
              satir.disSupheli = aracFark > 0 && aracFark <= menzil;
            }
          }
          // Anlık ortalama SADECE depo_full=true VE dış-yakıt DEĞİLSE hesaplanır.
          if (fark > 0 && h.depo_full && !disYakit) {
            // Sayaç tipi km ise lt/100km, saat ise lt/saat
            const carpan = arac?.sayac_tipi === "saat" ? 1 : 100;
  ...
  function verDialogDuzenleAc(y: AracYakit) {
    ...
    setVerDialogDisYakit(y.dis_yakit_oncesi ?? null);
    setVerDialogOpen(true);
  }
  ...
  async function duzKaydet() {
    // Düzeltme SADECE yöneticiye açık
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-3 mb-4 space-y-3">
        ...
        {yEkle && (
          <div className={`grid grid-cols-1 sm:grid-cols-3 ${isYonetici ? "lg:grid-cols-4" : ""} gap-2 w-full sm:w-auto`}>
            ...
            {/* Düzeltme — SADECE yönetici */}
            {isYonetici && (
              <Button className="bg-indigo-600 hover:bg-indigo-700 text-white h-11 text-sm" onClick={duzDialogAc}>
                <AlertTriangle size={16} className="mr-1.5" /> Düzeltme
              </Button>
            )}
          </div>
        )}
          ...
          <Table className="text-xs">
            <TableHeader className="!z-[50]">
              <TableRow className="bg-[#64748B] hover:bg-[#64748B]">
                ...
                <TableHead className="text-white text-[11px] px-2 text-right min-w-[70px] sticky top-0 z-[50] bg-[#64748B]">Fark</TableHead>
                <TableHead className="text-white text-[11px] px-2 text-right min-w-[90px] sticky top-0 z-[50] bg-[#64748B]">Miktar</TableHead>
                <TableHead className="text-white text-[11px] px-2 text-right min-w-[90px] sticky top-0 z-[50] bg-[#64748B]">Anlık Ort.</TableHead>
                <TableHead className="text-white text-[11px] px-2 text-right min-w-[90px] sticky top-0 z-[50] bg-[#64748B]">Genel Ort.</TableHead>
                <TableHead className="text-white text-[11px] px-2 text-right min-w-[70px] sticky top-0 z-[50] bg-[#0f2540]">Stok</TableHead>
                <TableHead className="text-white text-[11px] px-2 min-w-[120px] sticky top-0 z-[50] bg-[#64748B]">Kullanıcı Adı</TableHead>
                <TableHead className="text-white text-[11px] px-2 min-w-[120px] sticky top-0 z-[50] bg-[#64748B]">Not</TableHead>
                {(yDuzenle || ySil) && <TableHead className="text-white text-[11px] px-2 text-center w-[70px] sticky top-0 z-[50] bg-[#64748B]">İşlem</TableHead>}
              </TableRow>
            </TableHeader>
            <TableBody>
                    ...
                    aracKaynakText = (
                      <div>
                        <div className="font-bold flex items-center gap-1">
                          ...
                          {h.duzeltme && (
                            <span
                              className="text-[9px] bg-indigo-100 text-indigo-700 px-1 rounded font-bold"
                              title="Yakıt düzeltmesi — mazot dökülmesi/yazılmaması nedeniyle hisse oranında dağıtılan ± litre. Gerçek dolum değildir, tüketim ortalamasına katılmaz."
                            >
                              Düzeltme
                            </span>
                          )}
        ...
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto overflow-x-hidden">
          ...
          <div className="space-y-3 py-2 overflow-hidden">
            ...
            <div className="space-y-1 py-1">
              <Label className="text-xs text-gray-500">Dışarıdan yakıt</Label>
              <div className="grid grid-cols-3 gap-1">
                {([
                  { v: null as boolean | null, t: "Otomatik" },
                  { v: true as boolean | null, t: "Alındı" },
                  { v: false as boolean | null, t: "Alınmadı" },
                ]).map((o) => (
                  <button
                    key={String(o.v)}
                    type="button"
                    onClick={() => setVerDialogDisYakit(o.v)}
                    disabled={verDialogLoading}
                    className={`px-2 py-2 rounded-lg border-2 text-xs font-semibold transition-all ${
                      verDialogDisYakit === o.v
              ...
              <p className="text-[10px] text-gray-400 leading-snug break-words">
                Otomatik: fark 1 depo menzilini aşarsa dış yakıt sayılır (ortalamaya katılmaz).
                Alındı/Alınmadı: elle sabitler.
              </p>
            </div>
            <div className="space-y-1">
      ...
      {/* Yakıt Düzeltme Dialog — SADECE yönetici */}
      <Dialog open={duzDialogOpen} onOpenChange={setDuzDialogOpen}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto overflow-x-hidden">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
          ...
          <div className="space-y-3 py-2 overflow-hidden">
            ...
            <div className="space-y-1">
              ...
              <input
                type="text"
                inputMode="text"
                value={duzDialogMiktar}
                onChange={(e) => setDuzDialogMiktar(formatParaInput(e.target.value))}
                placeholder="Örn: -250 (eksik) veya 120 (fazla)"
                className={selectClass + " w-full"}
                disabled={duzDialogLoading}
              />
            </div>
            <div className="space-y-1">
              ...
              <input
                type="text"
                value={duzDialogNotu}
                onChange={(e) => setDuzDialogNotu(e.target.value)}
                placeholder="Ör: Mazot dökülmesi düzeltmesi"
                className={selectClass + " w-full"}
                disabled={duzDialogLoading}
              />
            </div>
            <div className="flex gap-2 justify-end pt-2">
              <Button variant="outline" onClick={() => setDuzDialogOpen(false)} disabled={duzDialogLoading}>İptal</Button>
              <Button
