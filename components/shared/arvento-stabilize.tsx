...
export default function ArventoStabilize({ bas, bitis, tekrarEsigi = 0, gridMesafe = 12, transitHiz = 20, mukerrerDk = 0, mukerrerYaricap = 0, kalinliklar, renkler, kamyonIziRenk = "#dc2626", kamyonIziKalinlik = 3, sekmeMap, canliKonumlar, canliCihazMap, gorunumRef: disGorunumRef, refreshKey = 0, sonGuncelleme, sonGuncellemeRapor = null, ocakLat = null, ocakLng = null, ocakYaricap = 150, yDuzenle = false, izinliPlakalar, katmanIzinli, canliButton, sekmePanel, kmlIndir = true, ocakMakineleri = [], ilkSonKontakMap }: { bas: string; bitis: string; tekrarEsigi?: number; gridMesafe?: number; transitHiz?: number; mukerrerDk?: number; mukerrerYaricap?: number; kalinliklar?: { reglaj?: number; serme?: number; silindir?: number }; renkler?: { reglaj?: string; serme?: string; silindir?: string }; kamyonIziRenk?: string; kamyonIziKalinlik?: number; sekmeMap?: SekmeAtamaMap; canliKonumlar?: CanliKonum[]; canliCihazMap?: CihazMap; gorunumRef?: MutableRefObject<HaritaGorunum | null>; refreshKey?: number; sonGuncelleme?: Date | null; sonGuncellemeRapor?: Date | null; ocakLat?: number | null; ocakLng?: number | null; ocakYaricap?: number; yDuzenle?: boolean; izinliPlakalar?: string[] | null; katmanIzinli?: KatmanIzin; canliButton?: ReactNode; sekmePanel?: ReactNode; kmlIndir?: boolean; ocakMakineleri?: { plaka: string; model: string | null; cins: string | null; calismaSn: number; lat: number | null; lng: number | null }[]; ilkSonKontakMap?: Map<string, { ilk: string | null; son: string | null; ilkT?: boolean; sonT?: boolean }> }) {
...
<kml xmlns="http://www.opengis.net/kml/2.2">
    ...
    return (
      <div className="space-y-3 harita-tamekran-kapsayici relative">
        <div className="text-center py-16 bg-white rounded-lg border">
          ...
          {/* Tarih navigatörü burada da erişilebilir olsun → kullanıcı sıkışmasın (tarihi değiştirebilsin). */}
          <div className="mt-4 max-w-[240px] mx-auto flex flex-col gap-1">{canliButton}</div>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-3 harita-tamekran-kapsayici relative">
      {/* Tam ekranda sol kenarda sekme paneli — normal modda gizli (globals.css) */}
      {sekmePanel}
