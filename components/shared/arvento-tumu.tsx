...
export default function ArventoTumu({ bas, bitis, tekrarEsigi = 0, silindirEsik = 0, gridMesafe = 12, transitHiz = 20, mukerrerDk = 0, mukerrerYaricap = 0, ocakLat = null, ocakLng = null, ocakYaricap = 150, damperSinif, kalinliklar, renkler, sekmeMap, canliKonumlar, canliCihazMap, gorunumRef: disGorunumRef, izinliPlakalar, katmanIzinli, refreshKey = 0, sonGuncelleme, canliButton, kmlIndir = true, calismaNoktalari, operasyonFiltre = null }: { bas: string; bitis: string; tekrarEsigi?: number; silindirEsik?: number; gridMesafe?: number; transitHiz?: number; mukerrerDk?: number; mukerrerYaricap?: number; ocakLat?: number | null; ocakLng?: number | null; ocakYaricap?: number; damperSinif?: Map<string, "gercek" | "mukerrer" | "ariza">; kalinliklar?: { reglaj?: number; serme?: number; silindir?: number }; renkler?: { reglaj?: string; serme?: string; silindir?: string }; sekmeMap?: SekmeAtamaMap; canliKonumlar?: CanliKonum[]; canliCihazMap?: CihazMap; gorunumRef?: MutableRefObject<HaritaGorunum | null>; izinliPlakalar?: string[] | null; katmanIzinli?: KatmanIzin; refreshKey?: number; sonGuncelleme?: Date | null; canliButton?: ReactNode; kmlIndir?: boolean; calismaNoktalari?: { plaka: string; rapor_tarihi: string; saat: string | null; lat: number; lng: number }[]; operasyonFiltre?: ArventoSekme[] | null }) {
  ...
  // Çoklu sekme seçimi: yalnız bu operasyonlar çizilir. null = filtre yok (klasik "Tümü").
  ...
  useEffect(() => {
    ...
    if (guzergahGoster) guzergahBirlesik.forEach((k) => {
      ...
      // ÇOKLU SEKME FİLTRESİ — yalnız seçilen operasyonlar çizilir. Filtre yoksa (null) davranış
      // eskisi gibi: o gün çalışan her araç çizilir. Sınıflama (op) DEĞİŞMEZ; filtre ayrı hesaplanır
      // ki filtresiz "Tümü" görünümü birebir aynı kalsın.
      if (filtreSeti) {
