...
export default function GidenEvrakForm({ evrak, onSuccess, onCancel }: Props) {
  ...
  const [kapatOnay, setKapatOnay] = useState(false); // düzenlemede ESC → "kaydedeyim mi?" onay dialog'u
  ...
  // ESC davranışı (Base UI Dialog escape-kapatmasını CAPTURE'da yakalayıp durdururuz):
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Evrak Taraması (PDF) bölümü kaldırıldı — artık her ek için ayrı PDF
          yüklendiğinden bu tek seferlik tarama gereksiz. Mevcut kayıtlardaki
          pdf_url alanı veritabanında dokunulmadan kalır; yeni evraklarda null'dur. */}
