// Banka yazışma form - Otomatik sayı no, çoklu ilgi/ek, kaşe toggle, ön izleme
"use client";
...
import {
  createBankaYazisma,
  updateBankaYazisma,
  getBankaYazismaSayiNo,
} from "@/lib/supabase/queries/banka-yazismalari";
import { getFirmalar } from "@/lib/supabase/queries/firmalar";
import { getBankaMuhataplarFull, createTanimlama } from "@/lib/supabase/queries/tanimlamalar";
...
type Props = {
  yazisma?: BankaYazismaWithRelations;
  onSuccess: () => void;
  onCancel: () => void;
};
...
export default function BankaYazismaForm({ yazisma, onSuccess, onCancel }: Props) {
  const isEdit = !!yazisma?.id;
  ...
  const [evrakSayiNo, setEvrakSayiNo] = useState(yazisma?.evrak_sayi_no ?? "");
  const [konu, setKonu] = useState(yazisma?.konu ?? "");
  const [muhatap, setMuhatap] = useState(yazisma?.muhatap ?? "");
  const [muhatapId, setMuhatapId] = useState(yazisma?.muhatap_id ?? "");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const no = await getBankaYazismaSayiNo(firmaId, muhatapId || null);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (pdfFile) {
        ...
        pdfUrl = await uploadDosya(pdfFile, "yazismalar", `banka/${firmaId}/${Date.now()}.pdf`);
      }
      ...
      if (isEdit) {
        await updateBankaYazisma(yazisma.id, payload);
        ...
        toast.success("Yazışma kaydedildi.");
      }
      onSuccess();
    } catch (err) {
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          // uploadDosya: boyut kontrolü + 413/JSON-olmayan yanıtı düzgün ele alır
                          const yeniUrl = await uploadDosya(dosya, "yazismalar", `banka-ek/${firmaId}/${Date.now()}-${temiz}`);
